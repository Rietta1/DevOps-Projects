...
### 7. 6. Open port 80 on the Ubuntu instance to allow access from the internet.
...
### 9. 1. Run ‘apt’ to acquire and install this software, run:
```
...
### 10. 3. Once installation is complete, log in to the MySQL console by running: 
```
sudo mysql
```
...
### 11. 4. Next, run a security script that comes pre-installed with MySQL, to remove some insecure default settings and lock down access to your database system. run: 
...
### 13. 5. Run the interactive script by typing:
```
...
### 16. 2. After installation is done, run the following command to confirm your PHP version: **php -v**
...
### 17. 4. At this point, your LAMP stack is completely installed and fully operational.
...
### 19. 2. assign ownership of the directory with your current system user, run:
``` sudo chown -R $USER:$USER /var/www/projectlamp
```



### 20. 3. Next, create and open a new configuration file in Apache’s sites-available directory. Type: sudo vi /etc/apache2/sites-available/projectlamp.conf
...
### 21. 4. This will create a new blank file. Paste in the following bare-bones configuration by hitting on I on the keyboard to enter the insert mode, and paste the text:
...
### 23. 5. To save and close the file. Hit the esc button on the keyboard, Type Type:wq! w for write and q for quit and Hit ENTER to save the file.
...
### 26.  8. Reload Apache so these changes take effect: 
```
...
### 27.  9. Finally, reload Apache so these changes take effect: 
```
 $ sudo systemctl reload apache2
```
...
### 28. 10. Next, use a2ensite command to enable the new virtual host:
 ```
  $ sudo a2ensite projectlamp
```
...
### 29. 11. Disable the default website that comes installed with Apache. type:
```
...
### 32.  14. The website is active, but the webroot/var/www/projectlamp is still empty. Create an index.html file in that location so that we can test that the virtual host works as expected:
...
### 43. This will open a blank file. Add the following text, which is valid PHP code, inside the file:
