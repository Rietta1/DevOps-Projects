...
resource "aws_subnet" "private" {
  count = var.preferred_number_of_public_subnets == null ? length(data.aws_availability_zones.available.names) : var.preferred_number_of_public_subnets 
  vpc_id                     = aws_vpc.main.id
  <!-- cidr_block                 = cidrsubnet(var.vpc_cidr, 8 , count.index)var.private_subnets[count.index + 2] -->
...
# Create the certificate using a wildcard for all the domains created in rietta.click
resource "aws_acm_certificate" "rietta" {
  domain_name       = "*.rietta.online"
  validation_method = "DNS"
}

# calling the hosted zone
data "aws_route53_zone" "rietta" {
  name         = "rietta.online"
  private_zone = false
}

# selecting validation method
resource "aws_route53_record" "rietta" {
  for_each = {
    for dvo in aws_acm_certificate.rietta.domain_validation_options : dvo.domain_name => {
  ...
  zone_id         = data.aws_route53_zone.rietta.zone_id
}

# validate the certificate through DNS method
resource "aws_acm_certificate_validation" "rietta" {
  certificate_arn         = aws_acm_certificate.rietta.arn
  validation_record_fqdns = [for record in aws_route53_record.rietta : record.fqdn]
}

# create records for tooling
resource "aws_route53_record" "tooling" {
  zone_id = data.aws_route53_zone.rietta.zone_id
  name    = "tooling.rietta.online"
  type    = "A"
...
resource "aws_lb_listener" "web-listener" {
  ...
  port              = 443
  protocol          = "HTTPS"
  certificate_arn   = aws_acm_certificate_validation.rietta.certificate_arn
...
```
# creating sns topic for all the auto scaling groups
resource "aws_sns_topic" "rietta-sns" {
name = "Default_CloudWatch_Alarms_Topic"
}

resource "aws_autoscaling_notification" "rietta_notifications" {
  group_names = [
    aws_autoscaling_group.bastion-asg.name,
    aws_autoscaling_group.nginx-asg.name,
    aws_autoscaling_group.wordpress-asg.name,
  ...
  topic_arn = aws_sns_topic.rietta-sns.arn
}
...
```
# launch template for WORDPRESS website
...
# launch template for TOOLING website
